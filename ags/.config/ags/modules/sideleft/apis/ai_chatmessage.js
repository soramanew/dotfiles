...
const { exec, execAsync, writeFile, timeout } = Utils;
...
execAsync(["bash", "-c", `rm ${LATEX_DIR}/*`])
    .then(() => execAsync(["bash", "-c", `mkdir -p ${LATEX_DIR}`]).catch(print))
    .catch(() => {});
const Latex = (content = "") => {
    const latexViewArea = Box({
        ...
        attribute: {
            render: (self, text) => {
                ...
                writeFile(text, filePath).catch(print);
...
`;
                writeFile(renderScript, scriptFilePath).catch(print);
        ...
        setup: self => self.attribute.render(self, content),
    });
    const wholeThing = Box({
        className: "sidebar-chat-latex",
        homogeneous: true,
        attribute: {
            updateText: text => latexViewArea.attribute.render(latexViewArea, text),
        },
        children: [
            Scrollable({
                vscroll: "never",
...
const Divider = () => Box({ className: "sidebar-chat-divider" });
                ...
                setup: self =>
                    self
                        .hook(
                            message,
                            (_, isThinking) => {
                                messageContentBox.toggleClassName("thinking", isThinking);
                            },
                            "notify::thinking"
                        )
                        .hook(
