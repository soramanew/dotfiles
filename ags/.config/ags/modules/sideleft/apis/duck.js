                ...
                Box({
                    ...
                    children: [
                        ...
                        ImageAction({
                            name: 'Hoard',
                            icon: 'save',
                            action: () => execAsync(['bash', '-c', `mkdir -p ~/Pictures/Ducks && cp ${thisBlock.attribute.imagePath} ~/Pictures/Ducks`]).catch(print),
                        }),
                        ImageAction({
    ...
    const thisBlock = Box({
        ...
        attribute: {
            ...
            'update': (imageData, force = false) => {
                ...
                const { status, signature, url, extension, source, dominant_color, width, height } = thisBlock.attribute.imageData;
                if (status != 200) {
