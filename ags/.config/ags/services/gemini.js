...
const initMessages = [
    ...
    {
        role: "user",
        parts: [{ text: '"He rushed to where the event was supposed to be hold, he didn\'t know it got calceled"' }],
    },
    {
        role: "model",
        parts: [
...
class GeminiMessage extends Service {
    ...
    parseSection() {
        ...
        if (!parsedData.candidates) this._parts[0].text += `Blocked: ${parsedData.promptFeedback.blockReason}`;
        else {
...
class GeminiService extends Service {
    ...
    set key(keyValue) {
        this._key = keyValue;
        Utils.writeFile(this._key, KEY_FILE_LOCATION).then(this.emit("hasKey", true)).catch(print);
    }
