...
parser.add_argument("--color", type=str, default=None, help="Generate colour scheme from colour")
...
parser.add_argument("--smart", action="store_true", default=False, help="Decide scheme type based on image colour")
parser.add_argument("--transparency", type=str, choices=["opaque", "transparent"], default="opaque", help="Enable transparency")
parser.add_argument("--termscheme", type=str, default=None, help="JSON file containg the terminal scheme for generating term colours")
parser.add_argument("--harmony", type=float , default=0.8, help="(0-1) Colour hue shift towards accent")
parser.add_argument("--harmonize_threshold", type=float , default=100, help="(0-180) Max threshold angle to limit colour hue shift")
...
parser.add_argument("--cache", type=str, default=None, help="File path to store the generated colour")
...
def harmonize(design_color: int, source_color: int, threshold: float = 35, harmony: float = 0.5) -> int:
    ...
    diff_degrees = difference_degrees(from_hct.hue, to_hct.hue)
    rotation_degrees = min(diff_degrees * harmony, threshold)
...
if not args.debug:
    ...
    print(f"$transparent: {str(transparent).lower()};")
